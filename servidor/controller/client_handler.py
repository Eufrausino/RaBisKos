...
class ClientHandler:
    ...
    def handle_client(self, client_socket, endereco) -> None:
        logging.debug(f"[NOVA LIGAÇÃO] {endereco} conectado.")
        try:
            while True:
                ...
                logging.debug(f"[REQUISIÇÃO] {mensagem.get('type')} de {endereco}")
            ...
            logging.debug(f"[ERRO] {endereco}: {e}")
        finally:
            ...
            client_socket.close()
            logging.debug(f"[DESCONECTADO] {endereco} fechado.")
